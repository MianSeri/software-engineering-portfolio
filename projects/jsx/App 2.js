function App ()
{
	...
	function getRandomStatus() {
		const randomIdx = Math.floor(Math.random() * observationStatuses.length);
		return observationStatuses[randomIdx];
	}
			...
			{spacePhenomena.map((item) => {
				...
				return (
					<div key={item.id} style={{ marginBottom: "15px"}}>
						<p>
							{item.emoji} <strong>{item.name}</strong> - {status}
						</p>

						{status.includes("Prime for Study") && (
							<p style={{color: "red", fontWeight: "bold" }}>
								Important! Bring your advanced equipment!
							</p>
						)}
					</div>
				);
